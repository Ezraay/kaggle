...
def simulate_games(agent1: Agent, agent2: Agent, num_games: int) -> dict:
    results = {}
    for i in range(num_games):
        game_result = simulate_game(agent1, agent2)
        if results[game_result] is None:
...
def simulate_game(agent1: Agent, agent2: Agent) -> GameState:
    board = Board()
    board.create((7, 6))
